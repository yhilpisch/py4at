...
class TestRunBacktest(TestCase):
    def setUp(self):
        ...
        self.real_data_dca_bot = DCABot(self.real_chart_data, 0.01, math.inf)
        self.real_data_dca_bot.get_signals()

        self.chart_data = generate_simpel_sample_momentum()
        self.test_dca_bot = DCABot(self.chart_data, 0.01, math.inf)
...
#@pytest.mark.skip("calcs to long for general test")
class TestRunBacktestLimit(TestCase):
    def setUp(self):
        self.real_chart_data = Three_commasDCA_safety_order_calc.get_data_from_file('2021-10-04', '2022-02-13',
                                                                                    "Binance_BTCUSDT_1h_format.csv")
        self.real_data_dca_bot = DCABot(self.real_chart_data, 0.01, 1000)
        self.real_data_dca_bot.get_signals()

    @parameterized.expand([
        [(1, 1.5, 0.1), (1, 2, 1), (1, 40, 1), (1, 1.5, 0.1), [1.1, 1, 18, 1], -18.89915],
    ])
    def test_real_back_overfitting(self, safety_order_volume_scale, price_deviation, max_safety_trades_count,
                                   safety_order_step_scale, expected_result, expected_amount):
        start_base_size = 10
